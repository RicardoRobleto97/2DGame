  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("assets/StarCommander1.wav");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load StarCommander1.wav")+Mix_GetError();

  startMusic();

	sounds.push_back( Mix_LoadWAV("assets/phaser.wav") );
  sounds.push_back( Mix_LoadWAV("assets/Boom.wav") );
  sounds.push_back( Mix_LoadWAV("assets/explosion.wav") );
  sounds.push_back( Mix_LoadWAV("assets/Larc.wav") );
